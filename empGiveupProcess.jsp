...
<jsp:useBean id="userInformation" scope="session" class="com.gm.HR.ValueObject.LoginUserValueObject" /> <!--//使用者的資訊 -->
<jsp:useBean id="formParameter" scope="request" class="com.gm.HR.CompetencyAssessment.pdf.AssessmentFormParameter" /> <!-- //資料摻數，引入的套件名稱 -->
<jsp:useBean id="assessmentForm" scope="request" class="com.gm.HR.CompetencyAssessment.pdf.AssessmentForm" /> <!-- //產生PDF檔 -->
<%
...
Statement stat = null;
String WK_TemplatePath = request.getRealPath("/template") + "/";   // 存放產生職務評核表PDF檔的資料夾/原始樣本的PDF檔
String WK_DocumentPath = request.getRealPath("/document") + "/";   // 存放產生職務評核表PDF檔的資料夾/產生後的PDF檔
int WK_Year = 0;
int WK_Month = 0;
int WK_Day = 0;
...
if(! WK_File.exists()){  //檢查File.txt是否存在
	try {
		// 該員工選擇放棄參加職能評核的更新選擇狀態處理
		WK_Command = "update competency_assessment_list set choose_time='" + WK_CDateTime + "', choose='N' where year=" + userInformation.getYear() + " and item=" + userInformation.getItem() + " and id='" + userInformation.getId() + "'";
		...
		if(conn != null) {
			...
			formParameter.setGiveUp(true);
			
			// 產生放棄職務評核表 pdf 檔
			assessmentForm.setSourcePDF(WK_TemplatePath + "/HR40E.pdf"); //產生的原始路徑 樣板
			assessmentForm.setTargetPDF(WK_DocumentPath + "/" + WK_FileName); //產生的目的
			assessmentForm.createAssessmentForm(formParameter); //產生
			
			out.println("產生放棄職務評核表 pdf 檔完成<br>");
			out.println("<button class='button' onclick=\"document.location.href='empHome.jsp'\">回本次評核畫面</button>");
			
			stat = null;
			conn = null;
		} else {
			out.println("系統查無此資料，請聯絡系統管理員");
		}
	} catch(Exception ex) {
		out.println(ex + "無法連接資料庫");
	}
} else {
	out.println("已產生過職務評核表 pdf 檔, 不可再次產生<br>");
	out.println("<button class='button' onclick=\"document.location.href='empHome.jsp'\">回本次評核畫面</button>");
}
%>